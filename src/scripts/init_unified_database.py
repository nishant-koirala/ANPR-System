...
def initialize_rbac_data(session):
    ...
    for role_name, description, permissions in default_roles:
        ...
        if not existing_role:
            ...
            existing_perms = {
                ...
                for rp in session.query(RolePermission).join(Permission).filter(RolePermission.role_id == existing_role.role_id).all()
            }
            for perm_name in permissions:
                perm_id = permission_map.get(perm_name)
                if perm_id and perm_name not in existing_perms:
                    session.add(RolePermission(role_id=existing_role.role_id, permission_id=perm_id))
    
    # Reassign legacy roles to the new model
    allowed_roles = set(role_map.keys())
    legacy_roles = session.query(Role).filter(~Role.role_name.in_(allowed_roles)).all()
    for legacy_role in legacy_roles:
        target_role_name = Roles.ADMIN if legacy_role.role_name.upper().startswith("SUPER") else Roles.OPERATOR
        target_role_id = role_map.get(target_role_name)
        if target_role_id:
            session.query(UserRole).filter(UserRole.role_id == legacy_role.role_id).update(
                {UserRole.role_id: target_role_id}
            )
        session.query(RolePermission).filter(RolePermission.role_id == legacy_role.role_id).delete()
        session.delete(legacy_role)
    ...
    if not existing_admin:
        ...
        if Roles.ADMIN in role_map:
            admin_role = UserRole(
                user_id=admin_user.user_id,
                role_id=role_map[Roles.ADMIN]
            )
